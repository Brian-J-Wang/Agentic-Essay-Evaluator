...
import ViewportContext from "@components/app/views/viewportContext";
		...
		essays: [
			{
				...
				dateCreated: new Date(),
				status: [1, 1, 0],
				essaySummary: "Thesis is present but lacks specificity. Evidence is cited but not analyzed in depth.",
				actionItems: ["Narrow thesis to a specific claim", "Add analysis after each cited source"],
			},
			{
				id: "s2",
				dateCreated: new Date(),
				status: [0, 1, 1],
				essaySummary: "Thesis is improved. Evidence analysis still needs more depth in body paragraphs 2 and 3.",
...
const AssignmentList: React.FC<AssignmentListViewport> = () => {
	...
	const handleAssignmentItemClick = (assignmentItem: AssignmentItem) => () => {
		viewportContext.setViewport({ type: "assignmentViewport", props: { assignmentItem } });
	};
